...
with st.sidebar:
    ...
    for module_id, port in MODULE_PORTS.items():
        ...
        dot = "🟢" if up else "🔴"
        st.markdown(
            f"{dot} `{module_id}` <span style='color:#94a3b8;font-size:11px'>:{port}</span>",
            unsafe_allow_html=True,
        )
...
tab_overview, tab_run, tab_traces, tab_tokens, tab_failures = st.tabs(
    ["Topology", "Run", "Traces", "Tokens & Cost", "Failure modes"]
)
...
with tab_overview:
    ...
    if topo.failed:
        ...
        for module_id, info in modules.items():
            up = health.get(module_id, False)
            with st.expander(
                f"{'🟢' if up else '🔴'}  {module_id} — {' → '.join(info.get('agents', []))}",
                expanded=False,
            ):
                left, right = st.columns([2, 1])
                with left:
                    st.markdown(f"**{info.get('service_id','')}** · `{info.get('url','')}`")
                    st.write(info.get("description", ""))
                    tags = "".join(
                        pill(cap, MODULE_COLOR.get(module_id, "#6366f1")) + " "
                ...
                with right:
                    st.markdown(
                        ...
                        else pill("DEPENDS ON " + ", ".join(info.get("depends_on", [])).upper(), "#f59e0b"),
                        unsafe_allow_html=True,
                    )
                    meta = client.module_metadata(module_id)
                    if meta.ok:
                        st.caption(f"model: `{meta.data.get('model','?')}`")
                        st.caption(f"deployment: `{meta.data.get('deployment_id','?')}`")
...
# ---------------------------------------------------------------------- run
with tab_run:
    st.subheader("Run a module")

    col_a, col_b, col_c = st.columns([2, 2, 1])
    with col_a:
        module_id = st.selectbox("Module", list(MODULE_PORTS), key="run_module")
    with col_b:
        failure_mode = st.selectbox("Failure mode", FAILURE_MODES, key="run_mode")
    with col_c:
        use_deps = st.toggle("Dependencies", value=True,
                             help="Off runs the module alone, without calling upstream modules.")

    user_input = st.text_area(
        "Input", value=SAMPLE_INPUTS.get(module_id, ""), height=90, key=f"input_{module_id}"
    )

    if st.button("Run", type="primary", disabled=not health.get(module_id, False)):
        with st.spinner(f"Running {module_id}…"):
            started = time.time()
            result = client.run_module(
                module_id, user_input, failure_mode=failure_mode, use_dependencies=use_deps
            )
            elapsed = time.time() - started
        st.session_state["last_run"] = {"result": result, "module": module_id, "elapsed": elapsed}
    ...
    if run:
        ...
        if result.failed and not body:
            ...
            st.markdown(
                ...
                f"<span style='color:#94a3b8;font-size:13px'>in {run['elapsed']:.1f}s</span>",
                unsafe_allow_html=True,
            )
            if body.get("error"):
                st.error(body["error"])
            ...
            m = st.columns(4)
            m[0].metric("HTTP", result.status_code or "—")
...
with tab_traces:
    ...
    with col1:
        trace_module = st.selectbox("Recorded by", list(MODULE_PORTS), key="trace_module")

    listing = client.traces(trace_module, limit=25)
    if listing.failed:
        st.warning(f"`{trace_module}` unreachable — {listing.error}")
    else:
        traces = listing.data.get("traces", [])
        if not traces:
            st.info("No traces recorded yet. Run a module from the **Run** tab.")
        else:
            options = {
            ...
            with col2:
                chosen = st.selectbox("Trace", list(options), key="trace_pick")
            trace = options[chosen]
            trace_id = trace["trace_id"]

            distributed = client.distributed_trace(trace_id)
            if distributed.ok:
                ...
                st.plotly_chart(
                    trace_waterfall(all_spans, f"Trace {trace_id[:16]}…"),
...
with tab_tokens:
    ...
    if not rows:
        ...
        c = st.columns(4)
        c[0].metric("Total tokens observed", f"{total:,}")
    ...
    if run and (run["result"].data or {}).get("latency"):
        st.divider()
        st.markdown("#### Last run breakdown")
...
with tab_failures:
    ...
    with col1:
        target = st.selectbox("Target module", list(MODULE_PORTS), key="fail_module")
        fail_input = st.text_input("Input", value=SAMPLE_INPUTS.get(target, "test"), key="fail_input")
    ...
    st.markdown("#### Fire a scenario")
    cols = st.columns(3)
    for i, mode in enumerate(FAILURE_MODES):
        with cols[i % 3]:
            st.markdown(f'<div class="card"><b>{mode}</b><br>'
                        f'<span style="font-size:12px;color:#94a3b8">{descriptions[mode]}</span></div>',
                        unsafe_allow_html=True)
            if st.button(f"Run `{mode}`", key=f"fire_{mode}",
                         disabled=not health.get(target, False), width='stretch'):
                timeout = 20.0 if mode == "timeout" else 300.0
                with st.spinner(f"{target} · {mode}…"):
                    started = time.time()
                    res = client.run_module(target, fail_input, failure_mode=mode, timeout=timeout)
                    elapsed = time.time() - started
                st.session_state["last_failure"] = {
                    "mode": mode, "result": res, "elapsed": elapsed, "module": target
                }

    outcome = st.session_state.get("last_failure")
    if outcome:
        st.divider()
        res, body = outcome["result"], (outcome["result"].data or {})
        st.markdown(
            f"### `{outcome['mode']}` on `{outcome['module']}` → "
            f"{status_pill(body.get('status', 'unknown'))} "
        ...
        if not body:
            ...
            c[2].metric("Tokens still counted", f"{(body.get('tokens') or {}).get('total_tokens', 0):,}")
            ...
            trace = client.trace(outcome["module"], body.get("trace_id", ""))
            if trace.ok:
                st.markdown(span_kind_legend(), unsafe_allow_html=True)
